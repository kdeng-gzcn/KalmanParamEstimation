...
_, A_list_MLE, Fnorm_list_MLE, Neg_Loglikelihood_list_MLE = model_MLE.parameter_estimation(num_iteration=100)
...
ax3.axhline(y=baseline, color=colors[4], linestyle='--', label=r'$ -\ell (A^{ture} \mid Y) $')
ax3.set_xlabel("t")
ax3.set_ylabel(r"$ -\ell (A^{(t)} \mid Y) $")
ax3.legend()

plt.tight_layout()
plt.savefig("./Result/GraphEM Results.pdf")
plt.show()
